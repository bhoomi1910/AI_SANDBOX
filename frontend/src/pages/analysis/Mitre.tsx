...
import { mitreTechniques, mitreTactics } from "@/data/deepdive";
import { cn } from "@/lib/utils";
import type { MitreTechnique } from "@/data/types";
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <span className="text-xs text-muted-foreground">{mitreTechniques.length} techniques · {mitreTactics.length} tactics</span>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-1 overflow-x-auto pb-2 no-scrollbar">
            {mitreTactics.map((tactic, i) => {
              const count = mitreTechniques.filter((t) => t.tactic === tactic).length;
              return (
                <div key={tactic} className="flex items-center">
                  ...
                  {i < mitreTactics.length - 1 && <div className="h-px w-4 shrink-0 bg-gradient-to-r from-primary/40 to-primary/10" />}
                </div>
              );
            })}
          </div>
      ...
      <div>
        ...
        <div className="grid grid-flow-col gap-3 overflow-x-auto pb-2 no-scrollbar" style={{ gridAutoColumns: "minmax(190px, 1fr)" }}>
          {mitreTactics.map((tactic) => {
              const techs = mitreTechniques.filter((t) => t.tactic === tactic);
              return (
